...
class PASMnet(nn.Module):
    ...
    def forward(self, x_left, x_right, max_disp=0):
        ...
        if self.training:
            ...
            disp_s3, valid_mask_s3 = self.output(cost_s3, max_disp // 4)
...
class PASMnet_v2(nn.Module):
    def __init__(self):
        super(PASMnet_v2, self).__init__()
        ###############################################################
        ## scale     #  1  #  1/2  #  1/4  #  1/8  #  1/16  #  1/32  ##
        ## channels  #  16 #  32   #  64   #  96   #  128   #  160   ##
        ###############################################################
        ...
        # Feature Extraction
        # self.hourglass = Hourglass([32, 64, 96, 128, 160])
        self.feature_extractor = FeatureEncoder_v1()

        # Cascaded Parallax-Attention Module
        # self.cas_pam = CascadedPAM([128, 96, 64])
        self.cas_pam = CascadedPAM([3, 3, 3])
    ...
    def forward(self, x_left, x_right, max_disp=0):
        ...
        (fea_left_s1, fea_left_s2, fea_left_s3), fea_refine = self.feature_extractor(x_left)
        (fea_right_s1, fea_right_s2, fea_right_s3), _       = self.feature_extractor(x_right)
...
# Just use downsampled images
class FeatureEncoder_v1(nn.Module):
    def __init__(self):
        super(FeatureEncoder_v1, self).__init__()

    def forward(self, x):
        fea_E0 = nn.functional.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=False) # scale: 1/2
        fea_E1 = nn.functional.interpolate(fea_E0, scale_factor=0.5, mode='bilinear', align_corners=False) # scale: 1/4
        fea_E2 = nn.functional.interpolate(fea_E1, scale_factor=0.5, mode='bilinear', align_corners=False) # scale: 1/8
        fea_E3 = nn.functional.interpolate(fea_E2, scale_factor=0.5, mode='bilinear', align_corners=False) # scale: 1/16

        return (fea_E3, fea_E2, fea_E1), fea_E1